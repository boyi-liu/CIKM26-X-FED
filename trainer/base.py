...
from utils.metric_utils import create_metric
...
class BaseClient(ABC):
    def __init__(self, id, args):
        ...
        self.optim = torch.optim.SGD(params=self.model.parameters(),
                                     ...
                                     momentum=0.9,
                                     weight_decay=1e-4)
        self.metric = create_metric(['loss', 'acc'])
    ...
    def model2tensor(self, params=None):
        ...
        return torch.cat([param.data.view(-1) for selected, param in zip(selected_params, self.model.parameters())
    ...
    def test_all(self):
        for client in self.clients:
            ...
            for k, v in self.metric.items():
                v.add_sub_processor(client.metric[k])

        ret_dict = {}
        for k, v in self.metric.items():
            ret_dict[k], ret_dict[f'{k}_std'] = v.avg_sub_processor()
            v.clear_sub_processor()
        return ret_dict